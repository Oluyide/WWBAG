    ...
    public class CommentController : ApiController
    {
        ...
        public HttpResponseMessage PostPostComment(PostComment postcomment)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                var usr = db.UserProfs.FirstOrDefault(x => x.Id == postcomment.CommentedBy);
                var ret = new
                {
                    CommentedBy = postcomment.CommentedBy,
                    CommentedByName = usr.UserName,
                    CommentedByAvatar = imgFolder + (String.IsNullOrEmpty(usr.AvatarExt) ? defaultAvatar : postcomment.CommentedBy + "." + postcomment.UserProf.AvatarExt),
